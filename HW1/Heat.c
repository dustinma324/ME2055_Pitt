...
#define DANG (REAL)(ANG1 + ANG2) / (( REAL ) NANG - 1.f)
...
void gaussMethod(REAL *phi, REAL *out)
{
    for (INT j = 1; j < NANG; j++) {
        for (INT i = 1; i < NR - 1; i++) {
            REAL a = (DR * DR * DANG * DANG * CONV * CONV) / (-2 * DANG * DANG * CONV * CONV - 2);
            REAL b = 1 / (2 * DR * DR);
            REAL c = 1 / (DR * DR);
            REAL d = 1 / (DR * DR * DANG * DANG * CONV * CONV);
            out[ IC ] = a
                        * (-QP3 / K - b * (phi[ IP1 ] - phi[ IM1 ]) - c * (phi[ IP1 ] + phi[ IM1 ])
                           - d * (phi[ JP1 ] + phi[ JM1 ]));
            phi[ IC ] = out[ IC ];
        }
    }
...
#else
void jacobiMethod(const REAL *phi, REAL *out)
{
    for (INT j = 1; j < NANG; j++) {
        for (INT i = 1; i < NR - 1; i++) {
            REAL a = (DR * DR * DANG * DANG * CONV * CONV) / (-2 * DANG * DANG * CONV * CONV - 2);
...
void boundaryConditions(REAL *phi, REAL *rGrid, REAL *angGrid)
{
    for (INT j = 1; j < NANG + 1; j++) {
        for (INT i = 0; i < NR; i++) {
            if (angGrid[ IC ] <= ANG1) {
                ...
                if (angGrid[ IC ] == ANG1 + ANG2) phi[ IC ] = T1; // T1 side
...
void verifyingPhysics(const REAL *phi, REAL *rGrid, REAL *angGrid, REAL *qPrime)
{
    ...
    for (INT j = 1; j < NANG + 1; j++) {
        for (INT i = 1; i < NR; i++) {
            ...
            if (angGrid[ IC ] == ANG1 + ANG2) { // 300K side - Straight
...
INT main(int argc, char *argv[])
{
    ...
    boundaryConditions(theta, rGrid, angGrid);
    ...
    while (RESIDUAL < diff) {
#if (GAUSS)
        gaussMethod(theta, theta_new);
#else
        jacobiMethod(theta, theta_new);
#endif
        boundaryConditions(theta_new, rGrid, angGrid);
